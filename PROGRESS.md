...
> 更新日期: 2026-07-13 | 版本: V1.0.0 | 状态: 一期功能完成，准备提交审核
...
| 文档 | 路径 | 版本 |
|------|------|------|
| 产品需求文档 | docs/01-PRD-产品需求文档.md | V1.0.0 |
| 技术可行性分析 | docs/02-技术可行性分析.md | V1.0.0 |
| 技术设计文档 | docs/03-技术设计文档.md | V1.0.0 |
| 视觉设计方案 | docs/04-视觉设计方案.md | v0.2 |
| 市场调研与推广策略 | docs/05-市场调研与推广策略.md | v0.2 |
| UI 交互设计规范 | docs/06-UI交互设计规范.md | V1.0.0 |
| UI 设计文档 | docs/07-UI设计文档-一期收官.md | V1.0.0 |
| UI 交互流程图 | docs/08-UI交互流程图.md | V1.0.0 |
...
- [x] sitemap.json存在
- [x] 版本号V1.0.0
- [x] svg-placeholder全部清除

### 7.2 研发交付待办（需在微信开发者工具/公众平台操作）

| 序号 | 事项 | 操作位置 | 说明 |
|------|------|----------|------|
| 1 | 隐私弹窗组件化 | app.json | 添加 `"usePrivacyCheck": true`，改用微信官方隐私弹窗组件（`button open-type="agreePrivacyAuthorization"`），替换现有隐私页 |
| 2 | 内容安全审核 | clothes-add 云函数 | 衣物上传前调用 `cloud.openapi.security.imgSecCheck` 审核图片，违规图片拒绝入库 |
| 3 | 服务器域名白名单 | 微信公众平台 > 开发管理 > 服务器域名 | request 合法域名添加：`https://nj6fr9cwdr.re.qweatherapi.com`（和风天气）、`https://api.xiaomimimo.com`（MiMo API） |
| 4 | 小程序类目 | 微信公众平台 > 设置 > 基本设置 > 服务类目 | 已选「丽人服务」 |
| 5 | 云函数部署 | 微信开发者工具 > 云函数目录 | 右键上传并部署：`outfit-recommend`、`user-login`（本次有代码变更） |
| 6 | 真机测试 | 微信开发者工具 > 编译 > 预览 | 扫码在真机上完整走一遍流程：登录 > 添加衣服 > 一键搭配 > 采纳/舍弃 > 定位刷新 |

### 7.3 审核风险提示

| 风险项 | 说明 | 建议 |
|--------|------|------|
| AI生成内容 | 搭配理由由MiMo生成，可能包含不当内容 | 已有基础过滤（emoji替换），建议增加敏感词二次过滤 |
| 隐私政策 | 需与官方隐私弹窗组件对接 | 审核前务必完成第1项改造 |
| 图片内容 | 用户上传的衣服图片可能违规 | 审核前务必完成第2项imgSecCheck |
## 8. 成本
...
V1.0.0 搭配详情页直接展示各单品原始照片（2×2网格），未做图片合成。后续版本计划用 Pillow 将多件衣服的抠图拼接成一张搭配效果图，提升视觉呈现。
